    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${days.length}, minmax(0, 1fr))` }}>
        {days.map((d) => {
          ...
          const label = d.label ?? `Día ${days.indexOf(d) + 1}`;
          ...
          return (
            ...
            >
              <span className="text-[11px] font-black uppercase tracking-wider leading-tight text-center">{label}</span>
              <span className={`text-[10px] font-semibold text-center leading-tight ${isActive ? "opacity-90" : "opacity-70"}`}>{sub}</span>
