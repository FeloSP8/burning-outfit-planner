...
- Cada resultado lleva **la descripción del campamento** debajo del nombre, recortada a dos líneas: para decidir si te acercas hace falta saber qué es, no solo cómo se llama
- Un campamento **marcado se despliega** y enseña la ficha entera: la descripción completa y **todos sus eventos** del listado oficial, con su tipo y sus horas. El nombre sigue llevando al plano y el desplegable va en un botón aparte — las dos cosas hacen falta y meterlas en un mismo clic obligaría a elegir
- La descripción se recorta a 400 caracteres en el servidor. Es lo que se lleva uno al playa: entera, un párrafo por cada uno de los más de mil campamentos engordaría el snapshot sin que nadie lea tanto. Y los eventos viajan al mapa en una versión mínima (`CampEvent`: título, tipo y horas) — enteros son cientos de kB de más en cada carga, y sin cobertura salen gratis porque ya están en el snapshot
